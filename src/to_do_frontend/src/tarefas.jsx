...
function tarefas() {
  ...
  const [tarefasAndamentoCount, setAndamentoCount] = useState(-1);
  const [tarefasConcluidasCount, setConcluidasCount] = useState(-1);
  ...
  async function totalTarefasEmAndamento(){
    setAndamentoCount(parseInt(await to_do_backend.totalTarefasEmAndamento()))
  }
  async function totalTarefasConcluidas(){
    setConcluidasCount(parseInt(await to_do_backend.totalTarefasConcluidas()))
  }
  ...
  return (
    ...
    <main class="mt-[30px] mx-[30px]">
        ...
        <div class="w-full p-4 bg-white border border-gray-200 rounded-lg shadow-sm sm:p-8 dark:bg-gray-800 dark:border-gray-700">
          ...
          {tarefasAndamentoCount > -1 && <p class='text-right font-bold'>TOTAL {tarefasAndamentoCount}</p>}
        <br/>
                    ...
                    { tarefas.filter((ta) => ta.concluida).map((ta) => (
                        <li class="py-3 sm:py-4">
                            <div class="flex items-center">
                                ...
                                <div class="flex-1 mihttp://127.0.0.1:4943/tarefas/http://127.0.0.1:4943/tarefas/n-w-0 ms-4">
                                    <p class="text-sm font-medium text-gray-900 truncate dark:text-white">
                                        { ta.categoria } 
                                    </p>
                                    <p class="text-sm text-gray-500 truncate dark:text-gray-400">
        ...
        {tarefasConcluidasCount > -1 && <p class='text-right font-bold'>TOTAL {tarefasConcluidasCount}</p>}
